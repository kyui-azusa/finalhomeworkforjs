...
2. 好处: 
   ...
   1. 添加按钮
   	...
   	```
   2. 注册事件
   在 `vue模板` 中给按钮注册事件使用 `@*` 而不是`on*`, 同样会在后文解释
...
<ul>
<li v-for="item in products">
商品名称: {{item.name}} 商品库存: <button @click="item.stock--">-</button>{{item.stock}}<button @click="item.stock++">+</button>
</li>
</ul>
</div>
<script>
...
{% raw %}
   <div id="app-ff-3">
       <ul>
           <li v-for="item in products">
               商品名称: {{item.name}} 商品库存: <button @click="changeStock(item, item.stock-1)">-</button>{{item.stock}}<button @click="changeStock(item, item.stock+1)">+</button>
           </li>
       </ul>
   </div>
   <script src="./vue.js"></script>
...
{% raw %}
   <div id="app-ff-5">
       <ul>
           <li v-for="item in products">
               商品名称: {{item.name}} 商品库存: <button @click="changeStock(item, item.stock-1)">-</button>{{item.stock?item.stock:'无货'}}<button @click="changeStock(item, item.stock+1)">+</button>
           </li>
       </ul>
   </div>
   <script src="./vue.js"></script>
